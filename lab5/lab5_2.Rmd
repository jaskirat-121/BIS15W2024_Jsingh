...
```{r}
fish %>%.  #shift command and m to get this
...
```{r}
fish %>% #select which data to use 
  select(lakeid, radii_length_mm) %>% #oull out variables of interest
  filter(lakeid=="AL" | lakeid=="AR") %>% #select specific lakes
  filter(radii_length_mm >2 & radii_length_mm <4) %>% #have values between 2 and 4 
