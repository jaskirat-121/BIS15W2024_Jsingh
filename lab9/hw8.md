...
```
## here() starts at C:/Users/singh/OneDrive/Desktop/BIS15W2024_Jsingh
```

The quotes show the folder structure from the root directory.

```r
sydneybeaches2 <-read_csv(here("homework", "data", "sydneybeaches.csv")) %>% clean_names()
```
...
The data is not "tidy" because each observation is listed multiple times. It is currently in long format. I know this because each variable is represented multiple times in the data. 

3. We are only interested in the variables site, date, and enterococci_cfu_100ml. Make a new object focused on these variables only. Name the object `sydneybeaches_long`


```r
sydneybeaches_long <- sydneybeaches2 %>% 
...
```r
dates_seperate <- sydneybeaches_long %>% 
