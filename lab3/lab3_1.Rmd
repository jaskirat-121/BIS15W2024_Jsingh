...
```{r}
hot_spring$depth <- c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)
hot_spring
```

4. Calculate the mean temperature of all of the temperature measurements.
```{r}
temp <- hot_spring[ ,1] #same command as data matrix 
mean(temp)

# or do mean(hot_spring$temp)
```

5. Save your hot springs data as a `.csv` file! Do not allow row names. 
```{r}
write.csv(hot_spring, "hot_spring.csv", row.names = FALSE)
```
