...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #reader is a command in tidyverse to show what package it came from...works w/o it as well
```
...
```{r}
select(fish, matches("a.+er")) #handy code for project
```
...
```{r}
mean(mammals$`max. life`). #cant be negative, means someone use -999 instead of using NA
```
...
```{r}
mammals_subset <- select(mammals_new, -"order", -"family")
```
...
```{r}
mammals_character <- select_if(mammals_new, ~!is.numeric(.))
```
