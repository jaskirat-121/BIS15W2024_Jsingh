...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "rosybrown1", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(color= 'blue', fill= 'black', alpha=0.3)+
  labs(title = "Homerange Data")
```
...
homerange %>% 
  mutate(range_category=case_when(log10.hra<3.65 ~ "very small",
                                  log10.hra>=3.65 & log10.hra<=4.59 ~ "small",log10.hra>4.59 & log10.hra<=6.02 ~ "medium", log10.hra>=6.02 ~ "large"))
```
