...
```{r}
msleep %>% 
  summarize(n_genra=n_distinct(vore))
```
...
```
NaN means there are no values for the species at all(no brainwt for any of the observations). NA means its missing some of the values. 
